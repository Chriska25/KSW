  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <AdminPageHeader
        ...
        actions={
          <Button variant="primary" size="sm" onClick={handleOpenAddModal} className="space-x-2 shrink-0">
      ...
      <Card className="p-4">
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
          ...
          <Select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className={`${selectClass} w-full md:w-auto`}>
          ...
          <Select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className={`${selectClass} w-full md:w-auto`}>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-border">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-h2">Formules & tarifs</CardTitle>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center text-muted-foreground text-sm">
              ...
              <button type="button" onClick={handleOpenAddModal} className="text-primary hover:underline">
                Créer la première
              </button>
            </div>
          ) : (
            <Table>
              ...
              <TableBody>
                {filteredServices.map((s) => (
                  <TableRow key={s.id}>
                    <TableCell>
                      <img src={s.coverImage} alt="" className="h-11 w-16 object-cover rounded-lg border border-border" />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="font-semibold text-foreground tabular-nums">{formatPrice(s.price)}</div>
                    ...
                    <TableCell className="text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><Clock className="h-3 w-3 text-primary" />{s.durationMinutes / 60}h</span>
                      <span className="flex items-center gap-1 mt-0.5"><Camera className="h-3 w-3 text-primary" />{s.photosCount} photos</span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="inline-flex justify-end gap-1">
                        <Button variant="ghost" size="icon" onClick={() => handleOpenEditModal(s)} className="h-8 w-8" aria-label="Modifier">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => setDeleteTarget(s)} className="h-8 w-8 text-destructive hover:text-destructive/80" aria-label="Supprimer">
      ...
      <AdminModal
        ...
        footer={
          <>
            ...
            <Button type="submit" form="prestation-form" variant="primary" disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Enregistrer'}
            </Button>
          </>
        }
      >
        <form id="prestation-form" onSubmit={handleSaveForm} className="space-y-5 text-sm">
          <div>
            <label className="text-caption font-medium text-muted-foreground block mb-1.5">Titre de l&apos;offre</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Type de prestation</label>
              <Select
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Publication</label>
              <Select
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Prix ({currencySymbol})</label>
              <Input type="number" required value={formData.price} onChange={(e) => setFormData({ ...formData, price: Number(e.target.value) })} />
            </div>
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Acompte (%)</label>
            ...
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Durée (min)</label>
            ...
            <div>
              <label className="text-caption font-medium text-muted-foreground block mb-1.5">Photos</label>
          ...
          <div className="p-4 rounded-lg border border-border bg-surface-muted space-y-3">
            <label className="text-foreground font-medium flex items-center gap-2 text-sm">
              <ImageIcon className="h-4 w-4 text-primary" /> Image de couverture
            </label>
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              {formData.coverImage ? (
                <div className="relative h-24 w-32 rounded-lg overflow-hidden border border-border shrink-0 group">
                  <img src={formData.coverImage} alt="" className="h-full w-full object-cover" />
                  <button
                    type="button"
                    onClick={() => setCropSrc(formData.coverImage)}
                    className="absolute inset-0 bg-background/70 opacity-0 group-hover:opacity-100 flex items-center justify-center text-foreground text-caption gap-1 transition-opacity"
                  >
                    <Crop className="h-3.5 w-3.5" /> Recadrer
                  </button>
                </div>
              ) : (
                <div className="h-24 w-32 rounded-lg border-2 border-dashed border-border flex items-center justify-center text-muted-foreground shrink-0">
      ...
      >
        <p className="text-sm text-muted-foreground">
          La formule <strong className="text-foreground">{deleteTarget?.title}</strong> sera définitivement retirée du catalogue public.
        </p>
      </AdminModal>
