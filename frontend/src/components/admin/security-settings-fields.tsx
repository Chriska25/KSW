    ...
    <div className="space-y-6">
      <Card className="glass-panel">
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            <CardTitle className="text-lg flex items-center">
              <Shield className="h-5 w-5 text-amber-400 mr-2" /> Vue d&apos;ensemble sécurité
            </CardTitle>
            <CardDescription>
        ...
        <CardContent className="space-y-4 text-xs">
          {overviewError && <p className="text-rose-400">{overviewError}</p>}
          {overviewLoading && !overview ? (
            <p className="text-zinc-500">Analyse en cours…</p>
          ) : overview ? (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-950">
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider mb-1">Environnement</div>
                  <div className="font-bold text-white capitalize">{overview.environment}</div>
                  <StatusBadge ok={overview.environment === 'production'} label={overview.environment === 'production' ? 'Production' : 'Dev / local'} />
                </div>
                <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-950">
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider mb-1">HTTPS</div>
                ...
                <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-950">
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider mb-1">JWT / Sessions</div>
                  <div className="font-bold text-white flex items-center gap-1.5">
                ...
                <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-950">
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider mb-1">Anti-bruteforce</div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 rounded-xl border border-zinc-800/80 bg-zinc-900/40 text-center">
                  <div className="text-lg font-black text-white">{overview.counts.staffUsers}</div>
                  <div className="text-[10px] text-zinc-500">Comptes staff</div>
                </div>
                <div className="p-3 rounded-xl border border-zinc-800/80 bg-zinc-900/40 text-center">
                  <div className="text-lg font-black text-white">{overview.counts.clients}</div>
                  <div className="text-[10px] text-zinc-500">Clients</div>
                </div>
                <div className="p-3 rounded-xl border border-zinc-800/80 bg-zinc-900/40 text-center">
                  <div className="text-lg font-black text-white">{overview.counts.privateGalleries}</div>
                  <div className="text-[10px] text-zinc-500">Galeries privées</div>
                </div>
                <div className="p-3 rounded-xl border border-zinc-800/80 bg-zinc-900/40 text-center">
                  <div className="text-lg font-black text-white">{overview.counts.bookings}</div>
                  <div className="text-[10px] text-zinc-500">Réservations</div>
                </div>
              </div>

              <div className="p-4 rounded-xl border border-amber-400/20 bg-amber-400/5 space-y-2">
      ...
      <Card className="glass-panel">
        <CardHeader>
          <CardTitle className="text-lg flex items-center">
            <Lock className="h-5 w-5 text-amber-400 mr-2" /> Authentification & sessions
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4 text-xs">
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950 flex items-center justify-between gap-4">
            <div>
              <div className="font-bold text-white flex items-center gap-2">
            ...
            <input
              ...
              className="h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-amber-400 focus:ring-amber-400 cursor-pointer shrink-0"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                className="max-w-xs"
              />
              <p className="text-[11px] text-zinc-500 mt-1">Appliquée aux comptes admin après enregistrement (1–168 h).</p>
            </div>
            <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950">
              <div className="font-semibold text-white mb-1">Protection connexion</div>
              ...
              <Link href="/admin/users" className="inline-flex mt-3 text-amber-400 hover:underline text-[11px] font-semibold">
      ...
      <Card className="glass-panel">
        <CardHeader>
          <CardTitle className="text-lg flex items-center">
            <Mail className="h-5 w-5 text-amber-400 mr-2" /> Emails transactionnels (SMTP)
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4 text-xs">
          ...
          {(settings.smtpHost?.includes('gmail.com') || settings.smtpUser?.includes('@gmail.com')) && (
            <div className="p-4 rounded-xl border border-amber-400/20 bg-amber-400/5 space-y-2">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <span className="text-zinc-300 block">API Gmail (HTTPS) — ports SMTP bloqués</span>
                ...
                <input
                  ...
                  className="h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-amber-400 shrink-0"
                />
              </div>
              {settings.gmailUseApi && (
                <div className="text-[11px] text-zinc-400 leading-relaxed pt-1 border-t border-amber-400/10">
                  {overview?.gmailApiConfigured ? (
                    ...
                    <>
                      <p className="text-amber-300 mb-2">
                        Ajoutez dans <code className="text-amber-400">.env</code> puis{' '}
                        <code className="text-amber-400">docker compose up -d backend</code> :
                      </p>
                      <ol className="list-decimal list-inside space-y-1 text-zinc-500">
                        ...
                        <li>
                          <strong className="text-amber-300">Obligatoire</strong> —{' '}
                          <a
                          ...
                          <strong className="text-zinc-400">{STUDIO_GMAIL_ADDRESS}</strong> (sinon erreur 403)
                        </li>
                        <li>
                          Script :{' '}
                          <code className="text-zinc-400">python3 backend/scripts/gmail_oauth_setup.py</code>{' '}
                          (connectez-vous avec {STUDIO_GMAIL_ADDRESS} uniquement)
                        </li>
                        <li>
                          Copier le refresh token dans{' '}
                          <code className="text-zinc-400">GMAIL_REFRESH_TOKEN</code> (+ CLIENT_ID, CLIENT_SECRET,
                          GMAIL_USE_API=true)
                        </li>
                      </ol>
                    </>
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="gold" size="sm" onClick={applyGmailPreset}>
              Gmail (envoi réel)
            </Button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-500 mt-1">
                Gmail : <strong className="text-zinc-400">587</strong> (ou 465 si 587 bloqué). Mailtrap :{' '}
                <strong className="text-zinc-400">2525</strong>.
              </p>
            </div>
            <div>
          ...
          <div className="p-3 rounded-xl border border-emerald-400/20 bg-emerald-400/5 text-[11px] text-zinc-400 leading-relaxed">
            <strong className="text-emerald-300">Gmail — {STUDIO_GMAIL_ADDRESS}</strong> — Cliquez « Gmail (envoi réel) »
            (compte + API HTTPS préactivés). Si vous voyez un timeout sur le port 465/587, cochez « API Gmail (HTTPS) »
            et configurez les variables <code className="text-amber-400">GMAIL_*</code> dans .env. Sinon : mot de passe
            d&apos;application via{' '}
            <a
          ...
          <div className="p-3 rounded-xl border border-zinc-700/80 bg-zinc-900/40 text-[11px] text-zinc-500 leading-relaxed">
            <strong className="text-zinc-400">Mailtrap</strong> — sandbox = tests sans livraison ; production = domaine
            vérifié + token API.
          </div>
          <div className="flex flex-col sm:flex-row gap-2 sm:items-end">
            <div className="flex-1">
              <label className="text-zinc-400 block mb-1">Email de test</label>
              <Input
                value={testEmailTo}
          ...
          {testEmailMessage && <p className="text-emerald-400 text-[11px]">{testEmailMessage}</p>}
          {testEmailError && <p className="text-rose-400 text-[11px]">{testEmailError}</p>}
          {overview && (
            <p className="text-[11px] text-zinc-500">
              Statut SMTP détecté :{' '}
              <span className={overview.smtpConfigured ? 'text-emerald-400' : 'text-amber-400'}>
      ...
      <Card className="glass-panel">
        <CardHeader>
          <CardTitle className="text-lg flex items-center">
            <ScrollText className="h-5 w-5 text-amber-400 mr-2" /> Journal d&apos;audit
          </CardTitle>
          <CardDescription>
      ...
      <Card className="glass-panel">
        <CardHeader>
          <CardTitle className="text-lg flex items-center">
            <Database className="h-5 w-5 text-amber-400 mr-2" /> Sauvegardes
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4 text-xs">
          {backupCounts && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {Object.entries(backupCounts).map(([key, count]) => (
                <div key={key} className="px-3 py-2 rounded-lg border border-zinc-800 bg-zinc-950">
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950 space-y-2">
            <div className="font-semibold text-white">Contenu de l&apos;export</div>
            ...
            {lastBackupLabel ? (
              <p className="text-amber-300 text-[11px]">Dernière sauvegarde locale : {lastBackupLabel}</p>
            ) : (
              <p className="text-zinc-500 text-[11px]">Aucune sauvegarde téléchargée depuis ce navigateur.</p>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              className="space-x-1.5 border-amber-400/30 text-amber-200 hover:bg-amber-400/10"
            >
          ...
          <div className="p-4 rounded-xl border border-dashed border-zinc-700 bg-zinc-900/40 space-y-1">
            ...
            <p className="text-zinc-500 text-[11px]">
              ...
              <code className="text-zinc-400">./scripts/backup.sh</code> (voir DEPLOYMENT.md).
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="glass-panel border-rose-500/20">
        <CardHeader>
          <CardTitle className="text-lg flex items-center text-rose-200">
            <Sparkles className="h-5 w-5 text-rose-400 mr-2" /> Zone sensible
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-3 text-xs">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div className="font-bold text-white">Imposer les données locales sur le serveur</div>
              <div className="text-zinc-400 text-[11px] mt-1">
            ...
            <Button
              ...
              size="sm"
              onClick={onSyncLocal}
              className="space-x-1.5 border-rose-500/30 text-rose-200 hover:bg-rose-500/10 shrink-0"
            >
