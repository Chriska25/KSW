...
export function ServicePackageCard({ pkg, index, isPopular }: ServicePackageCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'prestations-card group relative flex flex-col rounded-3xl overflow-hidden border bg-surface-muted/80',
        isPopular
          ? 'border-primary/50 ring-1 ring-primary/25 shadow-md'
          : 'border-border/90 hover:border-muted-foreground/40',
        visible && 'is-visible'
      )}
    >
      {isPopular && (
        <div className="absolute top-4 right-4 z-20 bg-primary text-primary-foreground text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
          Coup de cœur
        </div>
      )}
      ...
      <div className="relative h-52 sm:h-56 overflow-hidden">
        {pkg.coverImage ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-surface-muted to-background flex items-center justify-center">
        ...
        <div className="absolute bottom-4 left-4 right-4 z-10">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-foreground leading-tight">{pkg.title}</h3>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 p-6 space-y-5">
        <div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl sm:text-4xl font-extrabold text-foreground tabular-nums">{formatPrice(pkg.price)}</span>
            <span className="text-xs text-muted-foreground uppercase tracking-wider">TTC</span>
          </div>
          <p className="text-xs text-primary/90 font-mono mt-1.5">
          ...
          {invitation ? (
            <>
              ...
              <li className="flex items-start gap-2.5">
                <CheckCircle2 className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                <span>
                  <Share2 className="h-3.5 w-3.5 inline mr-1 text-muted-foreground" />
              ...
              <li className="flex items-start gap-2.5">
                <CheckCircle2 className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                <span>
                  <BarChart3 className="h-3.5 w-3.5 inline mr-1 text-muted-foreground" />
