...
> = {
  elegant: { gradient: 'from-zinc-900 to-zinc-950', accent: 'bg-amber-400', border: 'border-amber-400/40' },
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-zinc-500 text-xs mt-0.5">Choisissez le rendu de votre page publique.</p>
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                selected ? `${preview.border} ring-2 ring-amber-400/30` : 'border-zinc-800 hover:border-zinc-600'
              )}
            >
              ...
              <span className={cn('text-xs font-semibold', selected ? 'text-amber-300' : 'text-zinc-300')}>
                {opt.label}
              </span>
            </button>
          );
      ...
      {onPrimaryColorChange && (
        <div>
          <label className="text-zinc-400 text-xs block mb-1.5">Couleur d&apos;accent (optionnel)</label>
          <div className="flex items-center gap-3">
            <input
              type="color"
              value={primaryColor || '#d4af37'}
              onChange={(e) => onPrimaryColorChange(e.target.value)}
              className="h-10 w-14 rounded-lg border border-zinc-700 bg-zinc-900 cursor-pointer"
            />
            <span className="text-zinc-500 text-xs">Titres et éléments mis en valeur sur l&apos;invitation.</span>
          </div>
        </div>
      )}
    </div>
