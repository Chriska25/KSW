...
export function PublicHeader() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-surface-muted/85border-b border-border/80">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2">
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8 text-xs font-semibold text-foreground">
            {navLinks.map((link) => (
              <AppLink
                key={link.href}
                href={link.href}
                className={`transition-colors whitespace-nowrap ${
                  isNavActive(link.href, link.exact) ? 'text-primary font-bold' : 'hover:text-primary'
                }`}
              >
                {link.label}
              </AppLink>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            <AppLink href="/login" className="text-xs font-semibold text-muted-foreground hover:text-foreground hidden lg:block">
              Connexion
            </AppLink>
            <ButtonLink
              href="/reservation"
      ...
      {mobileMenuOpen && (
        <>
          <button
            ...
            className="md:hidden fixed inset-0 z-[48] bg-black/65 -[2px]"
            onClick={() => setMobileMenuOpen(false)}
          />
          <nav
            id="public-mobile-menu"
            aria-label="Navigation mobile"
            className="md:hidden fixed inset-x-0 top-16 bottom-0 z-[49] bg-surface-muted/98 border-b border-border overflow-y-auto overscroll-contain animate-in slide-in-from-top duration-200"
          >
            <div className="p-4 sm:p-6 space-y-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              <div className="flex flex-col space-y-2 text-sm font-semibold">
                ...
                <AppLink
                  href="/galerie-privee"
                  onClick={() => setMobileMenuOpen(false)}
                  className="py-3 px-3 rounded-xl bg-primary-muted text-primary flex items-center"
                >
                ...
                <AppLink
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="py-3 px-3 rounded-xl text-foreground hover:bg-surface-muted"
                >
                  Connexion
                </AppLink>
                <ButtonLink
              ...
              <div className="pt-4 border-t border-border">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground px-3 mb-2">Apparence</p>
