...
export function PrivateGalleryPhotoCard({
  ...
  return (
    <RevealPhotoCard
      ...
      withShine={false}
      className={cn(
        'aspect-[4/3] rounded-2xl border border-border/80 bg-surface-muted cursor-pointer',
    ...
    >
      ...
      <button
        ...
        className="absolute top-3 right-3 z-20 p-2.5 rounded-full bg-surface-muted/70 border border-border/80 text-foreground hover:text-primary hover:border-primary/30 transition-colors"
        aria-label={photo.isFavorite ? 'Retirer des favoris' : 'Ajouter aux favoris'}
      >
        <Heart className={cn('h-4 w-4', photo.isFavorite && 'fill-amber-400 text-primary')} />
      </button>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-4 bg-gradient-to-t from-zinc-950 via-zinc-950/60 to-transparent translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
        <p className="text-sm font-semibold text-foreground truncate">{photo.title}</p>
        {photo.albumName && (
          <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{photo.albumName}</p>
        )}
      </div>

      <div className="absolute top-3 left-3 z-20 h-9 w-9 rounded-full bg-surface-muted/70 border border-border/80 flex items-center justify-center scale-75 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-300">
